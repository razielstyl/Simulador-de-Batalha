...
function actions.build()
    ...
    local sonarAttack = {
        ...
        execute = function(playerData, creatureData)
            ...
            print(string.format("%s arrancou uma pedra do chão e tacou dando %d pontos de dano", creatureData.name, damage))
            local healthRate = math.floor((playerData.health / playerData.maxHealth) * 10)
            print(string.format("%s: %s", playerData.name, utils.getProgressBar(healthRate)))
        end
    }
    ...
    local waitAction = {
        ...
        execute = function(playerData, creatureData)
            -- Apresentar resultado como print
            print(string.format("%s esta observando o adversario, e não fez nada nesse turno.", creatureData.name))
        end
    }
